...
(Logger gLogger,  const string uffFile, const string engineFile, int batchSize, bool serializeEngine,
...
void FaceNetClassifier::predictSVM(cv::Mat frame, std::vector<struct Bbox> outputBbox, Ptr<SVM> svm) {
    getCroppedFacesAndAlign(frame, outputBbox); // ToDo align faces according to points
    preprocessFaces();
    for(int i = 0; i < m_croppedFaces.size(); i++) {
        doInference((float*)m_croppedFaces[i].faceMat.ptr<float>(0), m_output);
        cv::rectangle(frame, cv::Point(m_croppedFaces[i].y1, m_croppedFaces[i].x1), cv::Point(m_croppedFaces[i].y2, m_croppedFaces[i].x2), 
                        cv::Scalar(0,0,255), 2,8,0);
        Mat DataMat(1, 128, CV_32F, m_output);
        float response;
...
void FaceNetClassifier::getE(cv::Mat face, float emb[128]) {
    ...
    double stddev_pxl = stddev3.at<double>(0);
    cv::Mat image2;
    face.convertTo(image2, CV_64FC1);
    face = image2;
    // fix by peererror
    cv::Mat mat(4, 1, CV_64FC1);
    mat.at <double>(0, 0) = mean_pxl;
    ...
    face = face - mat;
    // end fix
    face = face / stddev_pxl;
    face.convertTo(image2, CV_32FC3);
    face = image2;
    doInference((float*)face.ptr<float>(0), emb);
    // cout<<"trong class"<<emb[120]<<endl;
}
